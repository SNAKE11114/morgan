...
void Homework2And3(quote& next_quote, ofstream& out, ofstream& passive, int& time, int& step,
	vector<order>& orderbook, map<string, Exchange>& exchanges, vector<int>& level1book) {
	passive << "\nstep " << time << ':' << endl;
	order order_to_fill;
...
void read_order_from_console(vector<order>& orderbook, istream& in, ostream& out) {
	...
	unsigned qua;
	bool new_side;
	int order_id = orderbook.size() + 2;
	while (1) {
		out << "please enter the order's side, symbol, price, quantity seperated by blanks:\n";
		in >> si >> symbol >> pr >> qua;
		if (in.fail()) {
			out << "Invalid order!\n";
			char cleaner;
